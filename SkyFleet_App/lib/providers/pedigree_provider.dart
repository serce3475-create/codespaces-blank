...
class PedigreeProvider with ChangeNotifier {
    ...
    try {
      ...
      if (passiveDoc.exists) {
        ...
        return BirdDisplayData(
          ...
          notlar: data['notlar'],
          fatherId: data['babaId'],  // ← DÜZELTME: 'fatherId' değil 'babaId' olmalı
          motherId: data['anneId'],  // ← DÜZELTME: 'motherId' değil 'anneId' olmalı
          renk: data['renk'],
  ...
  pw.Widget _buildPdfPedigreeTree(
    ...
    // DEBUG
    if (node.fatherHalkaNo != null) {
      debugPrint("DEBUG: fatherHalkaNo: ${node.fatherHalkaNo}, pedigreeMap'de bulundu: ${fatherNode != null}");
    }
    if (node.motherHalkaNo != null) {
      debugPrint("DEBUG: motherHalkaNo: ${node.motherHalkaNo}, pedigreeMap'de bulundu: ${motherNode != null}");
    }
  ...
  Future<void> generateAndSharePedigreePdf(BuildContext context, String rootHalkaNo) async {
    ...
    // DEBUG: Pedigri listesinde bulunan tüm halka numaralarını yazdır
    debugPrint("DEBUG: Pedigri listesinde ${_pedigreeListesi.length} öğe var:");
    for (var node in _pedigreeListesi) {
      debugPrint("  - halkaNo: ${node.halkaNo}, level: ${node.level}, fatherHalkaNo: ${node.fatherHalkaNo}, motherHalkaNo: ${node.motherHalkaNo}");
    }
    ...
    debugPrint("DEBUG: PedigreeMap oluşturuldu, ${pedigreeMap.length} öğe içeriyor");
